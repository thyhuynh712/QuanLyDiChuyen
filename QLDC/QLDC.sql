...
NGAYSINH SMALLDATETIME,
GIOITINH NVARCHAR(10),
MADD INT FOREIGN KEY (MADD) REFERENCES DIADIEM (MADD),
)
GO
...
MAND CHAR(12) FOREIGN KEY (MAND) REFERENCES NGUOIDAN (MAND),
MADIEMDI INT FOREIGN KEY (MADIEMDI) REFERENCES DIADIEM (MADD),
MADIEMDEN INT FOREIGN KEY (MADIEMDEN) REFERENCES DIADIEM (MADD),
...
BEGIN
    SET NOCOUNT ON;
    IF EXISTS (SELECT 1 FROM NGUOIDAN WHERE MAND = @MAND)
    BEGIN
...
--GO
--
CREATE PROC spGetAllXa
as
begin
	Select MAXA as N'Mã xã', TENXA as N'Tên xã', SOLUONGNB as N'Số lượng nhiễm bệnh'
	from XA
end
go
...
IF OBJECT_ID('BANGTAM', 'U') IS NOT NULL
    DROP TABLE BANGTAM
-- Create a temporary table to store the calculated results
CREATE TABLE BANGTAM (
    TotalPopulation INT,
    TotalInfected INT,
    TotalNonInfected INT
)
Go
-- Create a stored procedure to calculate the infection statistics
CREATE PROCEDURE CalculateInfectionStats
AS
BEGIN
    -- Insert data into the temporary table using the provided table relationships
    INSERT INTO BANGTAM ( TotalPopulation, TotalInfected, TotalNonInfected)
    SELECT
        COUNT(DISTINCT NGUOIDAN.MAND) AS TotalPopulation,
        COUNT(DISTINCT TINH.SOLUONGNB) AS TotalInfected,
        COUNT(DISTINCT NGUOIDAN.MAND) - COUNT(DISTINCT TINH.SOLUONGNB) AS TotalNonInfected
    FROM
        TINH
        LEFT JOIN HUYEN ON HUYEN.MATINH = TINH.MATINH
        LEFT JOIN XA ON XA.MAHUYEN = HUYEN.MAHUYEN
        LEFT JOIN DIADIEM ON DIADIEM.MAXA = XA.MAXA
        LEFT JOIN NGUOIDAN ON NGUOIDAN.MADD = DIADIEM.MADD
        LEFT JOIN THONGTINYTE ON THONGTINYTE.MATTYTE = NGUOIDAN.MAND

    -- Return the calculated infection statistics from the temporary table
    SELECT * FROM  BANGTAM
END
EXEC CalculateInfectionStats
-- Kiểm tra dữ liệu trong bảng tạm thời
SELECT * FROM BANGTAM